...
class Hyperparameters:
    ...
    eval_stride = int(
        os.environ.get("EVAL_STRIDE", 0)
    )  # 0 = non-overlapping; >0 = sliding window
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        if self.tie_embeddings:
            ...
            with torch.no_grad():
                U, S, V = torch.linalg.svd(
                    self.tok_emb.weight.data, full_matrices=False
                )
                target_S = (
                    S[0] * (1.0 / torch.arange(1, S.shape[0] + 1, dtype=S.dtype)) ** 0.5
                )
        ...
        for i, block in enumerate(self.blocks):
            with torch.no_grad():
                phase = torch.sigmoid(
                    torch.tensor(3.0 * (i / max(num_layers - 1, 1) - 0.5))
                )
                block.resid_mix.data[0] = phase * torch.ones(block.resid_mix.shape[1])
                block.resid_mix.data[1] = (1 - phase) * torch.ones(
                    block.resid_mix.shape[1]
                )
        ...
        for i in range(0, len(my_windows), eval_batch_seqs):
            ...
            for b in range(bs):
                ...
                loss = F.cross_entropy(
                    scored_logits.float(), scored_targets, reduction="sum"
                )
...
def main() -> None:
    ...
    if eval_sl != args.train_seq_len:
        ...
        log0(
            f"eval_seq_len:{eval_sl} (NTK-scaled RoPE, {val_tokens_eval.numel()} tokens)"
        )
    else:
    ...
    if eval_stride > 0:
        ...
        log0(
            f"Compiling forward_logits for sliding window eval (stride={eval_stride}, seq_len={eval_sl})..."
        )
        compiled_logits = torch.compile(base_model.forward_logits, dynamic=False)
        # Warmup compilation
        eval_batch_seqs = 256
        warmup_x = torch.zeros(
            eval_batch_seqs, eval_sl, dtype=torch.int64, device=device
        )
        base_model.eval()
