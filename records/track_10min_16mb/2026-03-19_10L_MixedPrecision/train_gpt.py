...
class Hyperparameters:
    ...
    prune_ratio = float(
        os.environ.get("PRUNE_RATIO", 0.0)
    )  # fraction of int8 range to prune (e.g. 0.1 = zero out |val| <= 12)
    int4_layers = os.environ.get(
        "INT4_LAYERS", ""
    )  # comma-separated layer indices for reduced precision (e.g. "3,4,5,6")
    int4_step = int(
        os.environ.get("INT4_STEP", 16)
    )  # rounding step: 2=int7, 4=int6, 8=int5, 16=int4
        ...
        for name in list(quant_obj.get("quantized", {}).keys()):
            ...
            if layer_num in int4_set:
                ...
                quant_obj["quantized"][name] = (
                    ((t.float() / step).round() * step).clamp(-127, 127).to(torch.int8)
                )
