...
def quantize_float_tensor_int6(t: Tensor) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        q = (
            torch.clamp((torch.round(q_raw / 4) * 4), -128, 124)
            .to(torch.int8)
            .contiguous()
        )
        return q, scale.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()
...
class BlockLARS:
    ...
    def __init__(
        ...
        self.blocks: dict[str, list[nn.Parameter]] = {
            "attn": [],
            "mlp": [],
            "other": [],
        }
    ...
    def step(self) -> None:
        for params in self.blocks.values():
            ...
            g_sq = sum(
                p.grad.data.norm().item() ** 2 for p in params if p.grad is not None
            )
            w_norm, g_norm = w_sq**0.5, g_sq**0.5
            if g_norm > 1e-8:
                scale = max(
                    self.min_scale, min(self.max_scale, self.trust * w_norm / g_norm)
                )
            else:
                scale = 1.0
            for p in params:
...
def eval_val_sliding(
    ...
    with torch.inference_mode():
        for batch_start in range(0, len(rank_positions), batch_size):
            ...
            x = torch.stack([val_tokens[p : p + seq_len] for p in batch_pos]).to(
...
def main() -> None:
    ...
    block_lars = (
        BlockLARS(
            base_model, trust=args.block_lars_trust, min_scale=args.block_lars_min_scale
        )
    ...
    if master_process:
        ...
        log0(
            f"Total submission size {compress_label}: {quant_file_bytes + code_bytes} bytes"
        )
