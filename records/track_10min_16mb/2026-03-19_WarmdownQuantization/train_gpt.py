...
def quantize_float_tensor(t: Tensor, bits: int = 8) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim == 2:
        ...
        q = (
            torch.clamp(torch.round(clipped / scale[:, None]), -max_val, max_val)
            .to(torch.int8)
            .contiguous()
        )
    ...
    scale = torch.tensor(
        clip_abs / float(max_val) if clip_abs > 0 else 1.0, dtype=torch.float32
    )
    q = (
        torch.clamp(
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if name.endswith("c_k.weight") and any(
            f"blocks.{i}." in name
            for i in range(num_layers_total - 2, num_layers_total)
        ):
            kept = t.to(dtype=torch.float16).contiguous()
            passthrough[name] = kept
            passthrough_orig_dtypes[name] = str(t.dtype).removeprefix("torch.")
    ...
    with torch.inference_mode():
        for start in my_starts:
            ...
            per_pos_loss = F.cross_entropy(
                suffix_logits, suffix_targets, reduction="none"
            )
...
def main() -> None:
    ...
    if args.eval_stride > 0:
        ...
        log0(
            f"final_sliding_window_exact val_loss:{s_val_loss:.8f} val_bpb:{s_val_bpb:.8f}"
        )
